...
const { TraceModuleOptionMapping } = require("lib/core/traceModuleOptionMapping");
...
FBTrace.get = function(moduleId) {
  ...
  var dbgOption = TraceModuleOptionMapping[moduleId];

  if (!dbgOption) {
    // In traceModuleOptionMapping.js, we allow specifying a wildcard at the
    // end of a path as a joker character. Specially useful for folders
    // specific to a feature (like lib/console/).
    for (let id in TraceModuleOptionMapping)
    {
      if (TraceModuleOptionMapping.hasOwnProperty(id) && id.endsWith("*") &&
        moduleId.startsWith(id.slice(0, -1))) {
        dbgOption = TraceModuleOptionMapping[id];
        break;
      }
    }
    if (!dbgOption)
    {
      FBTrace.sysout("can't find a mapping for this moduleId: " + moduleId);
      dbgOption = "DBG_OTHERS";
    }
